...
namespace
{
    ...
    EM_BOOL mouseCallback(int eventType, const EmscriptenMouseEvent* e, void* userData)
    {
        ...
        switch (eventType)
        {
            case EMSCRIPTEN_EVENT_MOUSEDOWN:
            {
                ...
                event.mouseButton.x      = e->targetX;
                event.mouseButton.y      = e->targetY;
                ...
                else if (e->button == 3)
                {
                    ...
                    mouseStatus[sf::Mouse::XButton1] = true;
                }
                else if (e->button == 4)
                {
                    event.mouseButton.button = sf::Mouse::XButton2;
                    mouseStatus[sf::Mouse::XButton2] = true;
                }
                else
                {
            ...
            case EMSCRIPTEN_EVENT_MOUSEMOVE:
            {
                ...
                event.mouseMove.x = e->targetX;
                event.mouseMove.y = e->targetY;
                window->pushHtmlEvent(event);
                return 1;
            }
        ...
        switch (eventType)
        {
            case EMSCRIPTEN_EVENT_WHEEL:
            {
                if (std::fabs(e->deltaY) > 0.0)
                {
                    ...
                    switch (e->deltaMode)
                    {
                    case DOM_DELTA_PIXEL: event.mouseWheelScroll.delta = -e->deltaY / 100.f; break;
                    case DOM_DELTA_LINE: event.mouseWheelScroll.delta = -e->deltaY / 3.f; break;
                    case DOM_DELTA_PAGE: event.mouseWheelScroll.delta = -e->deltaY * 80.f; break;
                    }
                    
                    event.mouseWheelScroll.x     = e->mouse.targetX;
                    event.mouseWheelScroll.y     = e->mouse.targetY;
                    window->pushHtmlEvent(event);
                }
        ...
        switch (eventType)
        {
            case EMSCRIPTEN_EVENT_TOUCHSTART:
            {
                ...
                for (int i = 0; i < e->numTouches; ++i)
                {
                    ...
                    event.touch.x      = e->touches[i].targetX;
                    event.touch.y      = e->touches[i].targetY;
                    window->pushHtmlEvent(event);
    ...
    void setCallbacks()
    {
        ...
        if (emscripten_set_keypress_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, NULL, true, keyCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set keypress callback" << std::endl;

        if (emscripten_set_keydown_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, NULL, true, keyCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set keydown callback" << std::endl;

        if (emscripten_set_keyup_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, NULL, true, keyCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set keyup callback" << std::endl;

        if (emscripten_set_click_callback("#canvas", NULL, true, mouseCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set click callback" << std::endl;

        if (emscripten_set_mousedown_callback("#canvas", NULL, true, mouseCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set mousedown callback" << std::endl;

        if (emscripten_set_mouseup_callback("#canvas", NULL, true, mouseCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set mouseup callback" << std::endl;

        if (emscripten_set_dblclick_callback("#canvas", NULL, true, mouseCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set dblclick callback" << std::endl;

        if (emscripten_set_mousemove_callback("#canvas", NULL, true, mouseCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set mousemove callback" << std::endl;

        if (emscripten_set_mouseenter_callback("#canvas", NULL, true, mouseCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set mouseenter callback" << std::endl;

        if (emscripten_set_mouseleave_callback("#canvas", NULL, true, mouseCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set mouseleave callback" << std::endl;

        if (emscripten_set_mouseover_callback("#canvas", NULL, true, mouseCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set mouseover callback" << std::endl;

        if (emscripten_set_mouseout_callback("#canvas", NULL, true, mouseCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set mouseout callback" << std::endl;

        if (emscripten_set_wheel_callback("#canvas", NULL, true, wheelCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set wheel callback" << std::endl;

        if (emscripten_set_resize_callback("#canvas", NULL, true, uieventCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set resize callback" << std::endl;

        if (emscripten_set_scroll_callback("#canvas", NULL, true, uieventCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set scroll callback" << std::endl;

        if (emscripten_set_blur_callback("#canvas", NULL, true, focuseventCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set blur callback" << std::endl;

        if (emscripten_set_focus_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, NULL, true, focuseventCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set focus callback" << std::endl;

        if (emscripten_set_focusin_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, NULL, true, focuseventCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set focusin callback" << std::endl;

        if (emscripten_set_focusout_callback(EMSCRIPTEN_EVENT_TARGET_WINDOW, NULL, true, focuseventCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set focusout callback" << std::endl;

        if (emscripten_set_deviceorientation_callback(NULL, true, deviceorientationCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set deviceorientation callback" << std::endl;

        if (emscripten_set_devicemotion_callback(NULL, true, devicemotionCallback) != EMSCRIPTEN_RESULT_SUCCESS)
            sf::err() << "Failed to set devicemotion callback" << std::endl;

        if (emscripten_set_orientationchange_callback(NULL, true, orientationchangeCallback) != EMSCRIPTEN_RESULT_SUCCESS)
